...
| Arquivo | Papel |
|---------|-------|
| `src/components/ModalVinculoMarketplace.tsx` | O modal principal. Um componente para os dois modos (`modo: 'listagem' \| 'venda'`) |
...
| DELETE | `/api/venda2/vinculoMarketplace` | Apaga vínculos (array, lote) |
...
alvos possíveis do vínculo (`produtoID`, `obsID`, `complementoID`, `produtoGrupoOpcID`,
...
3. **Modo venda esconde ações**: `Criar produto e vincular` e `Excluir` só renderizam com
   `modo === 'listagem'`.
...
6. **Bloqueio fiscal** (dois pontos, mesma regra):
   ...
   - **Só produto entra no filtro.** Opção pendente (`produtoGrupoOpcID`/`complementoID` nulos)
...
O próprio texto da tela diz o motivo: *"Pedidos de marketplace às vezes chegam com produtos
...
261 do tipo Grupo Opção; todas na filial 39202. Os 69 vínculos existentes eram **todos**
...
| 769 | AIQFome | 1 | 1 | O bloqueio fiscal (já estava paga, `valorPago = valorTotal`) |
...
Ou seja: **nasce ativo nos dois canais e sem preço**. O manual insiste em completar o cadastro
por causa disso.
...
pagamento, mas `venda.valorPago` continuou **0** — e a emissão fiscal, que olha `valorPago`,
...
- O campo `ignorar` (rota por venda) não tem UI.
