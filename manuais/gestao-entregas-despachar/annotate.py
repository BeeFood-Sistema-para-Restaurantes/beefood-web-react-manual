"""Anota as capturas do #107 — Despachar a rota e acompanhar.
...
e não para onde alguém achou que ele estava. Quem gerou os JSON: `/tmp/ge/cap107.py`, com
dois remendos (`fix107.py` e `fix107b.py`) para os dois alvos que mudam de nome **depois**
do despacho: o chip passa a dizer *Na rua* e o avião vira *Finalizar rota*.
...
# As etiquetas do painel caem em x=1300, sobre o mapa: a lateral direita tem 540 px de
# largura e texto em todas as linhas, então número em cima dela taparia endereço.

# --- 01 A rota pronta e o botão que dispara tudo ----------------------------
carregar("01-rota-pronta.png")
annotate("01-rota-pronta.png",
         marcadores=[
             (1, *alvo("chip_situacao", "esq"), 1300, 470),
             (2, *alvo("aviao", "esq"), 1300, 700),
             (3, *alvo("chip_em_rota", "baixo"), *etiqueta("chip_em_rota", "baixo", 110)),
         ],
         molduras=[moldura("cabecalho_rota"), moldura("aviao"), moldura("chip_em_rota")])

# --- 02 A janela de confirmação ---------------------------------------------
# Aqui o mapa foi escondido na captura, então a metade esquerda está vazia e recebe as
# etiquetas sem competir com nada.
carregar("02-confirmar-despacho.png")
annotate("02-confirmar-despacho.png",
         marcadores=[
             (1, *alvo("linha_entregador", "esq"), 610, 388),
             (2, *alvo("lista", "esq"), 610, 510),
             (3, *alvo("selo_preparo", "dir"), 1580, 452),
             (4, *alvo("aviso_preparo", "esq"), 610, 815),
             (5, *alvo("aviso_clientes", "esq"), 610, 940),
             (6, *alvo("confirmar", "dir"), 1580, 983),
         ],
         molduras=[moldura("aviso_preparo"), moldura("confirmar"),
                   moldura("selo_preparo")])

# --- 03 O que mudou no painel no segundo seguinte ---------------------------
carregar("03-rota-na-rua.png")
annotate("03-rota-na-rua.png",
         marcadores=[
             (1, *alvo("chip_situacao", "esq"), 1300, 430),
             (2, *alvo("selo_entregando", "esq"), 1300, 700),
             (3, *alvo("botao_finalizar", "esq"), 1300, 300),
             (4, *alvo("chip_em_rota", "baixo"), *etiqueta("chip_em_rota", "baixo", 110)),
             (5, *alvo("chip_preparacao", "baixo"), *etiqueta("chip_preparacao", "baixo", 110)),
         ],
         molduras=[moldura("cabecalho_rota"), moldura("chip_em_rota"),
                   moldura("chip_preparacao")])

# --- 04 O entregador andando ------------------------------------------------
carregar("04-entregador-na-rua.png")
annotate("04-entregador-na-rua.png",
         marcadores=[
             (1, *alvo("pin_entregador"), 700, 300),
             (2, *alvo("contagem", "esq"), 1300, 430),
             (3, *alvo("selo_entregando", "esq"), 1300, 560),
             (4, *alvo("rodape_entregadores", "esq"), 1300, 1180),
         ],
         molduras=[moldura("rodape_entregadores")])

# --- 05 A lista de entregadores com ele em rota -----------------------------
carregar("05-lista-em-rota.png")
annotate("05-lista-em-rota.png",
         marcadores=[
             (1, *alvo("grupo_em_rota", "esq"), 600, 487),
             (2, *alvo("linha_entregador", "esq"), 600, 620),
         ],
         molduras=[moldura("linha_entregador")])

# --- 06 O menu da rota já despachada ----------------------------------------
carregar("06-menu-na-rua.png")
annotate("06-menu-na-rua.png",
         marcadores=[
             (1, *alvo("tres_pontos", "esq"), 1300, 430),
             (2, *alvo("item0", "esq"), 1300, 560),
             (3, *alvo("item1", "esq"), 1300, 690),
             (4, *alvo("item2", "esq"), 1300, 820),
         ],
         molduras=[moldura("menu")])

# --- 07 Trocar o entregador no meio da rua ----------------------------------
carregar("07-trocar-entregador.png")
annotate("07-trocar-entregador.png",
         marcadores=[
             (1, *alvo("sem_entregador", "esq"), 600, 511),
             (2, *alvo("atual", "esq"), 600, 640),
             (3, *alvo("aviso", "esq"), 600, 849),
             (4, *alvo("salvar", "dir"), 1580, 927),
         ],
         molduras=[moldura("atual"), moldura("aviso"), moldura("salvar")])
