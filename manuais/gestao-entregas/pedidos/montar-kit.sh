...
# A skill (`.cursor/skills/`) também não viaja: o critério que ela guarda interessa a quem escreve
# manual, não a quem fotografa. O link vira texto para não apontar para o vazio.
sed -i 's#\[memória geral\](\.\./\.\./\.\./\.cursor/[^)]*)#memória geral do processo de manual#g' \
    "$KIT/1-pedido/README.md"
